...
public class Traversals {
 ...
 public static <T> String buildPostOrderString(TreeNode<T> node) {
  ...
  public static <T> List<T> collectLevelOrderValues(TreeNode<T> node) {
    ...
    list.add(node.value);
    collectLevelOrderValues(node, list);
    return list;
  }

  private static <T> void collectLevelOrderValues(TreeNode<T> node, List<T> list) {
    if(node == null){
      return;
    }
    if(node.left != null){
    ...
    collectLevelOrderValues(node.left, list);
    collectLevelOrderValues(node.right, list);
  }
  ...
  public static int countDistinctValues(TreeNode<Integer> node) {
    ...
    countDistinctValues(node, seen);
    return seen.size();
  }

  private static void countDistinctValues(TreeNode<Integer> node, Set<Integer> seen){
    if(node == null){
      return;
    }
 
    seen.add(node.value);
    countDistinctValues(node.left, seen);
    countDistinctValues(node.right, seen);
  }
  ...
  public static boolean hasStrictlyIncreasingPath(TreeNode<Integer> node) {
   ...
   public static boolean hasStrictlyIncreasingPath(TreeNode<Integer> node, int parentValue) {
      if(node == null){
        return true;
      }
